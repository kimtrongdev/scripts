...
async function handleBraveSetting (action) {
  ...
  if (action.is_show_ui) {
    ...
    await updateUserInput(action.pid,'CLICK', 1332, 781,0,0,"",'click')
    // select option
    await updateUserInput(action.pid,'CLICK', 1322, 805,0,0,"",'click')
    // click Trackers & ads blocking
    await updateUserInput(action.pid,'CLICK', 1338, 636,0,0,"",'click')
    // select option
    await updateUserInput(action.pid,'CLICK', 1313, 694,0,0,"",'click')
  } else {
    //click Fingerprinting blocking
