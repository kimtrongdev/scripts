        ...
        if(action.suggest){
            ...
            if(action.other_videos.length >= CHANNEL_VIDEO_WATCH){
                ...
                if (IS_MOBILE) {
                    ...
                    if (!next) {
                        next = document.querySelectorAll('ytm-item-section-renderer[section-identifier="related-items"] ytm-video-with-context-renderer').item(randomNext)
                    }
                    ...
                    let susggestVideo = next.querySelector('a#thumbnail')
                    await userClick(action.pid, `${nextSelector} a#thumbnail`, susggestVideo)
                }
