...
async function startChromeAction(action, _browser) {
    ...
    let exs = ["ex", "quality", "extensions/canvas", "extensions/audio", "extensions/font", "extensions/webgl"]
    let level_name = ''
    ...
    let userAgent = ''
    if (systemConfig.uas) {
        userAgent = systemConfig.uas.split('\n')
        userAgent = utils.randomRanger(0, userAgent.length - 1)
        if (userAgent) {
...
async function getScriptData(pid, isNewProxy = false) {
    ...
    if (action) {
        ...
        if(action.mobile_percent === undefined || action.mobile_percent === null){
            ...
            if (['watch', 'watch_video', 'comment_youtube', 'like_fb_page', 'like_fb_post'].includes(action.id)) {
